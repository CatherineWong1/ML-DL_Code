...
class CRF(nn.Module):
    ...
    def _data_to_likelihood(self, rolls):
        """
        convers a numpy array of rolls (integers) to log-likelihood.
        :param rolls: [1, n_rolls]
        :return:
        """
    ...
    def _compute_likelihood_numerator(self, loglikelihoods, states):
        ...
        We'll iterate over the sequence of states and compute the sum of the
        relevant transition cost with the log likelihood of the observedroll.
            Input:
                loglikelihoods: torch Variable. Matrix of n_obs * n_states.
    ...
    def _compute_likelihood_denominator(self, loglikelihhods):
        """
        ...
        prev_alpha = self.transition[:, self.n_states] + loglikelihhods[0].view(1, -1)

        for roll in loglikelihhods[1:]:
            alpha_t = []

            for next_state in range(self.n_states):
                feature_function = self.transition[next_state, :self.n_states].view(1, -1) + roll[next_state].view(1, -1).expand(1, self.n_states)
